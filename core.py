...
def main():
    ...
    if not os.path.isfile(config_path):
        ...
        log.fatal("A config file has been created."
                  " Customize it, then restart 3be-telegram!")
        exit(1)
    ...
    with open("config/template_config.toml", encoding="utf8") as template_cfg_file, \
        ...
        if not template_cfg.cmplog(user_cfg):
            log.fatal("There were errors while parsing the config file. Please fix them and restart 3be-telegram!")
            exit(2)
        else:
    ...
    me = bot.get_me()
    if me is None:
        logging.fatal("The token you have entered in the config file is invalid. Fix it, then restart 3be-telegram.")
        sys.exit(1)
